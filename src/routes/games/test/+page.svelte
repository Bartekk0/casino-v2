<script lang="ts">
  ...
  // REAKTYWNE wyliczenie mnożnika - aktualizuje się na zmianę chance albo rollAbove
...
<main>
  ...
  <div class="options">
    <label for="chanceRange">Szansa na wygraną: {chance}%</label>
    <input
      type="range"
      id="chanceRange"
      min="1"
    ...
    <label class="checkbox-label">
      <input
        type="checkbox"
        bind:checked={rollAbove}
        disabled={loading}
        on:change={() => {
          // Odwróć etykietę dynamicznie
        }}
      />
      {rollAbove ? 'Higher (≥)' : 'Lower (≤)'}
    </label>

    <div class="multiplier">Mnożnik: x{multiplier.toFixed(2)}</div>
  ...
  {#if message}
    <div class="info">
      <div>{message}</div>
      <div>Rzut: {roll}</div>
      <div>Wypłata: {win ? formatMoney(payout) + ' zł' : '-'}</div>
    </div>
  {/if}
</main>
